...
worcestercounty <- subset(county, NAME = "COUNTYFP" , drop = FALSE)

censusTractDf <- left_join(censusTractPop, vehicle, by = "ct10_id") #joining data frames
censusTractdf_2 <- polygonCensusTract %>%  
    mutate(ct10_id = as.numeric(ct10_id)) # convert type to numeric
censusTractDf1 <- left_join(censusTractdf_2, censusTractDf, by = "ct10_id") #joining new dataset
...
Density_plot 
v  <- 4326
st_transformDensity <- st_transform(DensitySfObj, crs = v)
st_county <- st_transform(county, crs = v)
Worcester_county <- st_county %>% filter (COUNTYFP == 027)
st_county_Density <- sf::st_intersects(st_county, st_transformDensity)
unlisted_Dw <- unlist(st_county_Density)
Worcester_Density_Df <-as.data.frame(unlisted_Dw)
Worcester_Density_Censis_Tract <-suppressWarnings(plot(,  col = "blue", pch = 20, add = TRUE)) 
```
# join
