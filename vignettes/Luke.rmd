...
#Cropping
#reproject
routes_reprojected <- st_transform(x = routes, crs = st_crs(worcester)) # reproject to same CRS
stops_reprojected  <- st_transform(x = stops, crs = st_crs(worcester)) # reproject to same CRS
roads_reprojected  <- st_transform(x = roads, crs = st_crs(worcester)) # reproject to same CRS
#major roads in Worcester
roads_worc <- st_intersection(x = roads_reprojected , y = worcester) #roads worcester intersect
#bus stops
stops_worc <- st_intersection(x = stops_reprojected, y = worcester) # stops worcester intersect
#bus routes
routes_worc <- st_intersection(x = routes_reprojected, y = worcester) # routes worcester intersect
censusTractDataFram1 <- left_join(censusTractPop, vehicle, by = "ct10_id")  # join by census tract id
st)
censusDataVPOP <- polygonCensusTract %>%  # Polygon convert character to numeric column
...
worcester_census <- st_intersection(x = censusDataVPOPPoly, y = worcester)  #census worcester data 
popDensityTable <- worcester_census[ , c("pop", "area_acres", "area_sqft", "ct10_id", "objectid", "shape")] 
popDensityTable #pop density data worcester
vehicleDensityTable <- worcester_census[ , c("veh_tot", "area_acres", "area_sqft", "ct10_id", "objectid", "shape")] 
vehicleDensityTable # vehicle density data worcester
...
plot(routes_worc)
vehicleDensityTable_subset <- subset(vehicleDensityTable, select = c("density_sqmile", "geometry")) #subset to make datasets smaller
...
par(mar = c(0, 0, 1, 0))
set.seed(1) # plotting
Population_Density <- plot(popDensityTable_subset,  col = c("orange", "red", "bisque"))
title("Density")
Vehicle_Density <- plot(vehicleDensityTable_subset, col = c("green", "blue", "aquamarine"))
title("Density")
```
