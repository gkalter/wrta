...
#Reading in Data
```{r, eval = FALSE}
ridership <- system.file("extdata/Weekday_ridership.csv", package = "wrta") %>% st_read
...
#clean data
routes_c <- routes %>% select(BusRoute, Shape_Leng, geometry) 

ridershipd <- as.numeric(ridership)

#completing a join via coercion
route_ridership <- left_join(x = as.numeric(ridershipd), y = routes_c, by = c("ROUTE", "BusRoute")) 
